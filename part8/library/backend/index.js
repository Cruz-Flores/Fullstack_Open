...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      if (args.author && !args.genre) {
        ...
        const booksByGenreAndAuthor = byGenre.filter(
  ...
  Author: {
    bookCount: (root) => {
      const bookCount = books.filter((b) => root.name === b.author);
  ...
  Mutation: {
    addBook: (root, args) => {
      ...
      const authorIncluded = authors
        .filter((b) => b.name === args.author)
    ...
    editAuthor: (root, args) => {
      ...
      authors = authors.map((a) => (a.name === author.name ? updateAuthor : a));
      return updateAuthor;
    },
  },
};
